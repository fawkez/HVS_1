...
MagWarning = 'Gaia G_RVS band apparent magnitude must be computed to select stars visible in the Gaia radial velocity catalogue. Calculating...'

S5Warning = 'Gaia G band and DECam g and r band apparent magnitudes must be computed to select stars within the S5 foot print. Calculating...'
...
#@subsample
def subsample(self, cut=None, use_rvs_sf=False):
            ...
            '''
                ...
                if self.size > 0:
                    ...
                    if use_rvs_sf:
                        ...
                        self.obsprob = dr2rvssf.query(cc, g=self.Gaia_G, 
                ...
                if self.size>0:
                    if use_rvs_sf:
                        ...
                        self.obsvar = dr3rvssfvar.queryvar(cc, g=self.Gaia_G, 
                                                c=self.Gaia_G - self.Gaia_RP)

                        #Retrieve Gaiaunlimited k (number of stars with vad)
                        #and Gaiaunlimited n (total number of stars)
                        self.k = dr3rvssfvar.queryk(cc, g=self.Gaia_G, 
                                                c=self.Gaia_G - self.Gaia_RP)
                        self.n = dr3rvssfvar.queryn(cc, g=self.Gaia_G, 
                                                c=self.Gaia_G - self.Gaia_RP)
                ...
                #idx8 = (np.abs(self.vlos)>800*u.km/u.s) 
