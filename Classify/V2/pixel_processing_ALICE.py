...
from V2.prepare_gaia_iterative import prepare_gaia_iterative, prepare_speedystar
...
def process_path(path, healpix_pixel):
    ...
    prior_path = os.path.join(os.path.dirname(simulation_path), 'Prior_map.csv')
...
if __name__ == '__main__':
    ...
    output_path = os.path.join(main_data_path, 'candidates_90_v2')
    ...
    all_pixels = np.arange(0, npix + 1)
    local_pixels = all_pixels[rank::size]
    print(f"Rank {rank} processing {len(local_pixels)} out of {len(all_pixels)} total pixels.")
