...
class EjectionModel:
    ...
    def set_imf(self):
        ...
        #User-specified imf
        elif callable(self.imftype) or isinstance(self.imftype, (np.ndarray, np.generic)):
            return self.imftype
        else:
    ...
    def _inverse_cumulative_mp(self, IMF, x):
        ...
        #if IMF == 'array'
            #mps = np.genfromtxt(IMF)[:,0]
            #print(self.imftype)
            #print(np.max(self.imftype))
...
class Hills(EjectionModel):
    ...
    '''
    HVS ejection model from Rossi+ 2017. Isotropic ejection from GC (by default) and with a mass/velocity distribution based on MC. Can generate an ejection sample using a Monte Carlo approach based on inverse transform 
    sampling. See also Marchetti+ 2018, Evans+2021, Evans+2022.
    '''
    ...
    def __init__(self, M_BH=4e6*u.Msun, Met=np.random.uniform, Zsun=0.0142,
        ...
        imftype: 'Salpeter' or 'Kroupa' or callable or file path string.
            ...
            Alternatively, a complete IMF defined by the imf package can 
            be passed explicitly and stellar masses are drawn from it.
            Alternatively, the file path of text file consisting of a single column of stellar masses can be supplied. This column will be read in and stellar masses will be sampled from it at random. Allows for more flexibility in the stellar mass.
    ...
    def sampler(self):
        ...
        if self.LaunchLoc is not None:
            ...
            #print(LaunchLocTime[0])
            if LaunchLocTime[0] != 0 or LaunchLocTime[1] - LaunchLocTime[0] > 0*u.Myr:
            ...
            #if -np.max(tflight) < np.min(LaunchLocTime) :
            #    raise ValueError("Error: maximum flight time is larger than maximum backwards flight time of launching location. Consider setting tflightmax to a smaller time or integrating the LaunchLoc orbit longer.")

            #Ejected from LaunchLoc. Initialize cartesian coordinates
            x0 = np.zeros(n)*u.kpc
            y0 = np.zeros(n)*u.kpc
            z0 = np.zeros(n)*u.kpc

            theta02 = np.zeros(n)*u.rad
            thetav02 = np.zeros(n)*u.rad
            phi02 = np.zeros(n)*u.rad
            phiv02 = np.zeros(n)*u.rad
            ...
            for j in range(n):
                ...
                #index = np.argmin(np.abs(ts-tflight[j]))
                
                #Initial position HVSs from sphere of influence around a cluster
                ...
                #print(self.LaunchLoc.time(quantity=True))
                #print(-tflight[j], zGC[j], self.LaunchLoc.z(-tflight[j],#quantity=True), z0[j])
                #print(np.sqrt(x0[j]**2 + y0[j]**2 + z0[j]**2))
                #Fix with atan2
                ...
                #if x0[j]>0:
                #        phi02[j] = np.arctan(y0[j]/x0[j])
                #elif x0[j]<0 and y0[j]>0:
                #        phi02[j] = np.arctan(y0[j]/x0[j]) + np.pi*u.rad
                #else:
                #        phi02[j] = np.arctan(y0[j]/x0[j]) - np.pi*u.rad
                #if z0[j]>0:
                #        theta02[j] = np.arctan(np.sqrt(x0[j]**2 + y0[j]**2)/z0[j])
                #else:
                #        theta02[j] = np.arctan(np.sqrt(x0[j]**2 + y0[j]**2)/z0[j]) +np.pi*u.rad
                ...
                #if vx0[j]>0:
                #        phiv02[j] = np.arctan(vy0[j]/vx0[j])
                #elif vx0[j]<0 and vy0[j]>0:
                #        phiv02[j] = np.arctan(vy0[j]/vx0[j]) + np.pi*u.rad
                #else:
                #        phiv02[j] = np.arctan(vy0[j]/vx0[j]) - np.pi*u.rad
                #if vz0[j]>0:
                #        thetav02[j] = np.arctan(np.sqrt(vx0[j]**2 + vy0[j]**2)/vz0[j])
                #else:
                #        thetav02[j] = np.arctan(np.sqrt(vx0[j]**2 + vy0[j]**2)/vz0[j]) +np.pi*u.rad
...
class HillsFromCatalog(Hills):
    """
    HVS ejection model that uses a pre-generated catalog of stars stored
    as an Astropy Table or a dictionary. This model skips IMF sampling
    and stellar evolution steps, directly using the input catalog.
    """

    """
    HVS ejection model from Rossi+ 2017, modified to use a pre-generated
    catalog of stars. This model skips the IMF sampling and stellar
    evolution steps, instead directly using input star properties to 
    compute ejection parameters. The properties include stellar mass, 
    age, radius, metallicity, photometry, and evolutionary stage.

    The ejection parameters (e.g., velocity, orbital period, separation)
    are calculated using the same methods as the original Hills model
    to ensure consistency.

    Parameters:
    ----------
    catalog : dict
        A dictionary containing arrays of pre-generated star properties.
        The required keys are:
        - 'mass': Stellar mass in solar masses.
        - 'age': Stellar age in Myr.
        - 'flight_time': Time since ejection in Myr.
        - 'radius': Stellar radius in solar radii.
        - 'logL': Logarithmic luminosity (L/Lsun).
        - 'Teff': Effective temperature in Kelvin.
        - 'logg': Surface gravity (log10(cgs)).
        - 'feh': Metallicity ([Fe/H]).
        - 'G': Absolute Gaia G-band magnitude.
        - 'BP': Absolute Gaia BP magnitude.
        - 'RP': Absolute Gaia RP magnitude.
        - 'eep': Evolutionary phase number (optional).

    Other parameters are inherited from the parent Hills model.
    """
    def __init__(self, catalog, **kwargs):
        """
        Initialize the HillsFromCatalog model with a catalog.

        Parameters:
        ----------
        catalog : astropy.table.Table or dict
            A pre-generated catalog of star properties. If an Astropy Table
            is provided, it will be converted to a dictionary for internal use.
        kwargs : dict
            Additional parameters passed to the parent Hills class.
        """
        super().__init__(**kwargs)
        # Convert Astropy Table to dictionary with numpy arrays
        if isinstance(catalog, Table):
            catalog = {col: np.array(catalog[col]) for col in catalog.colnames}
    ...
    def sampler(self):
        """
        Samples from the pre-generated catalog to generate an ejection sample.

        The catalog directly provides stellar properties (mass, age, radius,
        metallicity, etc.), which are then used to compute binary and ejection
        properties (e.g., orbital period, separation, ejection velocity).
        
        All calculations involving ejection kinematics and binary properties 
        follow the original Hills model.

        Returns:
        --------
        dict:
            Dictionary containing ejection parameters, including:
            - Initial position (r0, phi0, theta0)
            - Velocity components (v0, phiv0, thetav0)
            - Stellar properties (mass, age, radius, etc.)
            - Binary parameters (q, sep, P)
        """
        ...
        BP = self.catalog['BP']
        RP = self.catalog['RP']
        logg = self.catalog['logg']
        feh = self.catalog['feh']
        eep = self.catalog['eep']
        ...
        uniform_for_q = np.random.uniform(0, 1, n_catalog)
        q = self._inverse_cumulative_q(uniform_for_q, mass)

        # Calculate orbital period P and separation sep
        uniform_for_P = np.random.uniform(0, 1, n_catalog)
        P = self._inverse_cumulative_logP(uniform_for_P, radius, mass * (1 + q))
        P = P.to('day')

        sep = np.cbrt((const.G * (mass * (1 + q)) * P**2) / (4 * np.pi**2)).to('Rsun')

        # Calculate ejection velocities
        v0 = (np.sqrt(2 * const.G.cgs * (mass * q) / sep) * 
            (self.M_BH / (mass + mass * q))**(1./6.)).to('km/s')

        # Assign isotropic ejection angles
        phi0 = np.random.uniform(0, 2 * np.pi, n_catalog) * u.rad
        theta0 = np.arccos(np.random.uniform(-1, 1, n_catalog)) * u.rad
        phiv0 = phi0.copy()
        thetav0 = theta0.copy()
        ...
        stagebefore = eep  # Can assign a previous stage if applicable
        mem = np.ones(n_catalog, dtype=int)  # Membership identifier

        # Other required attributes
        r0 = np.ones(n_catalog) * self.centralr
        size = n_catalog
        ...
        return {
            ...
            'Zsun': self.Zsun,
         # Comment everything after this if it breaks
            'G': G,
            'BP': BP,
            'RP': RP,
            'logg': logg
