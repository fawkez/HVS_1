...
def photometry(self, bands=['Bessell_V', 'Bessell_I', 
                   ...
                   errors = ['e_par', 'e_pmra', 'e_pmdec', 'e_vlos'], data_release='DR4', extiction = False):
    ...
    if(self.size==0):
        ...
        else: # no extinction, set self.Av to 0 and call get_Mags
            self.Av, Mags, errs = get_Mags(0, self.dist.to('kpc').value, l,
                                        b, self.m.to('Msun').value, self.met,
                                        self.T_eff.value, self.Rad.value, 
                                        self.Lum.value, None, bands, errors)
