...
def add_unit_if_missing(value, unit):
    """Attach a unit to the value if it doesn't already have one."""
...
def remove_units(data):
    """
    Recursively remove units from the input.
    
    If data is an Astropy Quantity, return its numerical value.
    If data is an iterable (list, tuple, or numpy array), apply this
    ...
    if isinstance(data, u.Quantity):
        ...
        return {key: remove_units(val) for key, val in data.items()}
    elif isinstance(data, (list, tuple)):
        # Process lists and tuples recursively
        return type(data)(remove_units(item) for item in data)
    elif isinstance(data, np.ndarray):
        # If the numpy array has an object dtype, process element-wise.
        if data.dtype == np.object_:
            return np.array([remove_units(item) for item in data])
        else:
            # For numeric arrays that are already unitless, return as is.
            return data
    else:
        # Return the data unchanged if it is not iterable or a Quantity.
        return data
...
def implied_calculations(data):
    ...
    # check that ra_rad, dec_rad, pmra_rad_s, pmdec_rad_s, epmra_rad_s, epmdec_rad_s have the same length and do not have units
    assert len(ra_rad) == len(dec_rad) == len(pmra_rad_s) == len(pmdec_rad_s) == len(epmra_rad_s) == len(epmdec_rad_s), "The input arrays must have the same length"

    # check if the inputs have units and remove them
    ra_rad = np.array(remove_units(ra_rad))
    dec_rad = np.array(remove_units(dec_rad))
    pmra_rad_s = np.array(remove_units(pmra_rad_s))
    pmdec_rad_s = np.array(remove_units(pmdec_rad_s))
    epmra_rad_s = np.array(remove_units(epmra_rad_s))
    epmdec_rad_s = np.array(remove_units(epmdec_rad_s))
    ...
    plx_mas = 1000 / D_pc # 1/pc should give arcsec, convert to mas
    eplx_mas = 1000*eD_pc / D_pc**2 # error propagation for 1/x, it should be in mas
